...
A researched master list of **307 events** (304 published) — Missouri covered statewide via a
...
- `VendingCircuit.csv` — authoritative flat master, 307 events, 16 columns (added `county` + per-row `last_verified`)
...
`vending_published_events` (304 rows). Public-read RLS on `vending_*` only; accounting tables

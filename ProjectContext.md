...
A researched master list of **263 events** (260 published) — Missouri covered statewide via a
county-by-county sweep, plus regional metros incl. Indianapolis and the I-70/I-74 corridors (29 market
...
- `VendingCircuit.csv` — authoritative flat master, 263 events, 16 columns (added `county` + per-row `last_verified`)
...
`vending_published_events` (260 rows). Public-read RLS on `vending_*` only; accounting tables

...
A researched master list of **301 events** (298 published) — Missouri covered statewide via a
...
- `VendingCircuit.csv` — authoritative flat master, 301 events, 16 columns (added `county` + per-row `last_verified`)
...
`vending_published_events` (298 rows). Public-read RLS on `vending_*` only; accounting tables

...
A researched master list of **280 events** (277 published) — Missouri covered statewide via a
...
- `VendingCircuit.csv` — authoritative flat master, 280 events, 16 columns (added `county` + per-row `last_verified`)
...
`vending_published_events` (277 rows). Public-read RLS on `vending_*` only; accounting tables

...
A researched master list of **209 events** (206 published) — growing via an ongoing Missouri
county-by-county sweep (24 market hubs) — feeds a live Supabase database and a
...
- `VendingCircuit.csv` — authoritative flat master, 209 events, 16 columns (added `county` + per-row `last_verified`)
...
`vending_published_events` (206 rows). Public-read RLS on `vending_*` only; accounting tables

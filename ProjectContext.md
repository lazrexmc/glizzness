...
A researched master list of **328 events** (325 published) — Missouri covered statewide via a
...
- `VendingCircuit.csv` — authoritative flat master, 328 events, 16 columns (added `county` + per-row `last_verified`)
...
`vending_published_events` (325 rows). Public-read RLS on `vending_*` only; accounting tables

...
A researched master list of **317 events** (314 published) — Missouri covered statewide via a
...
- `VendingCircuit.csv` — authoritative flat master, 317 events, 16 columns (added `county` + per-row `last_verified`)
...
`vending_published_events` (314 rows). Public-read RLS on `vending_*` only; accounting tables

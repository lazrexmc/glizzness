...
A researched master list of **292 events** (289 published) — Missouri covered statewide via a
...
- `VendingCircuit.csv` — authoritative flat master, 292 events, 16 columns (added `county` + per-row `last_verified`)
...
`vending_published_events` (289 rows). Public-read RLS on `vending_*` only; accounting tables

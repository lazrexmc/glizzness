...
A researched master list of **190 events** (187 published) — growing via an ongoing Missouri
county-by-county sweep (21 market hubs) — feeds a live Supabase database and a
...
- `VendingCircuit.csv` — authoritative flat master, 190 events, 16 columns (added `county` + per-row `last_verified`)
...
`vending_published_events` (187 rows). Public-read RLS on `vending_*` only; accounting tables

...
A researched master list of **225 events** (222 published) — Missouri covered statewide via a
county-by-county sweep (26 market hubs) — feeds a live Supabase database and a
...
- `VendingCircuit.csv` — authoritative flat master, 225 events, 16 columns (added `county` + per-row `last_verified`)
...
`vending_published_events` (222 rows). Public-read RLS on `vending_*` only; accounting tables

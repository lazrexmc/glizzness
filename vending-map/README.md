...
## Embed on glizzness.com (GoDaddy Website Builder)

`embed.html` is a single self-contained file (Leaflet via CDN; `config.js` + `app.js` inlined)
generated by `python build_embed.py`. Regenerate it whenever `index.html`/`app.js`/`config.js` change.

**Path A — paste directly (no hosting):** In the GoDaddy editor add a section → **Custom Code / HTML**,
paste the entire contents of `embed.html`, publish. GoDaddy renders custom code in a sandboxed
iframe that runs scripts, so the map should load. Give the section a tall height.

**Path B — host + iframe (fallback if Path A is sandboxed/cramped):** host this folder free
(Netlify/Cloudflare/Pages), then paste a one-liner into a Custom Code block:
```html
<iframe src="https://YOUR-HOST/vending-map/" style="width:100%;height:80vh;border:0" title="Vending Circuit"></iframe>
```
The map still appears on glizzness.com; it just runs from the host inside the frame.

**Branded subdomain (optional, nicest):** host the folder, then in GoDaddy DNS add a CNAME
`festivals` → your host; the map lives at `festivals.glizzness.com` and you link to it from the
site menu. (Netlify/Cloudflare auto-issue the HTTPS cert.)
